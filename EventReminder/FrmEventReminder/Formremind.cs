    ...
    public partial class Formremind : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string query = "insert into Event_Reminder1(flight_No,event_description,event_date,event_time,event_remind,event_status) values('" + Fno + "','" + EventDes + "','" + EventDate + "','" + EventTime + "','" + remain + "','" + RemainDefault + "')";
                if (EC.Save_Update_DeleteEvent(query) == true)
                {
                    MessageBox.Show("Event sadded successfully");
                    Clear();
        ...
        private void cmbdefault_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Check == "Customize")
            {
                ...
                //cmbCustomize.Items.Insert(0, "-Select-");
                cmbCustomize.SelectedIndex = 0;
            }
            else
            {
                txtNum.Visible = false;
        ...
        {
            if (txtFlightNo.Text.Length == 0 || txtEvent.Text.Length == 0 || dtpTime.Text.Length == 0 || dtpdate.Text.Length == 0 || cmbdefault.Text.Length == 0)
            {
                MessageBox.Show("Some fields are empty" ,"Error" , MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }          
            else
